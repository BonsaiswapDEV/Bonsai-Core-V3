...
import { TestBonsaiswapV3SwapPay } from '../typechain/TestBonsaiswapV3SwapPay'
...
import { TestBonsaiswapV3ReentrantCallee } from '../typechain/TestBonsaiswapV3ReentrantCallee'
...
describe('BonsaiswapV3Pool', () => {
  ...
  let factory: BonsaiswapV3Factory
  let pool: MockTimeBonsaiswapV3Pool

  let swapTarget: TestBonsaiswapV3Callee
  ...
  async function initializeAtZeroTick(pool: MockTimeBonsaiswapV3Pool): Promise<void> {
    await pool.initialize(encodePriceSqrt(1, 1))
  ...
  // https://github.com/Bonsaiswap/bonsaiswap-v3-core/issues/214
  ...
  describe('#lock', () => {
    ...
    it('cannot reenter from swap callback', async () => {
      const reentrant = (await (
        await ethers.getContractFactory('TestBonsaiswapV3ReentrantCallee')
      ).deploy()) as TestBonsaiswapV3ReentrantCallee
  ...
  describe('swap underpayment tests', () => {
    let underpay: TestBonsaiswapV3SwapPay
    beforeEach('deploy swap test', async () => {
      const underpayFactory = await ethers.getContractFactory('TestBonsaiswapV3SwapPay')
      underpay = (await underpayFactory.deploy()) as TestBonsaiswapV3SwapPay
